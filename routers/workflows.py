...
from responses import WorkflowResponse, ListResponse
...
@router.get(
    "/workflows", 
    response_model=ListResponse[WorkflowResponse],
    responses={
        200: {"description": "List of available workflows"}
    }
)
@version(1)
async def list_workflows(service: WorkflowServiceDep):
    return {
        "object": "list",
...
@version(1)
async def get_workflow_by_id(workflow_id: str, service: WorkflowServiceDep):
